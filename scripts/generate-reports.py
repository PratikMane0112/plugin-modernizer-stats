...
# ── Derive additional fields for summary.json ────────────────────────────────

total_plugins = df["pluginName"].nunique()
successful_migrations = total_migrations - failed_migrations_count
pending_migrations = len(df[~df["migrationStatus"].isin(["success", "fail"])])

# Timeline: monthly success/fail breakdown
valid_ts = df.dropna(subset=["timestamp"])
valid_ts = valid_ts[valid_ts["timestamp"] != pd.NaT]
timeline_data = []
if not valid_ts.empty:
    valid_ts_copy = valid_ts.copy()
    valid_ts_copy["month"] = valid_ts_copy["timestamp"].dt.to_period("M").astype(str)
    for month, group in valid_ts_copy.groupby("month"):
        s = len(group[group["migrationStatus"] == "success"])
        f = len(group[group["migrationStatus"] == "fail"])
        timeline_data.append({"month": str(month), "success": s, "fail": f, "total": s + f})
    timeline_data.sort(key=lambda x: x["month"])

# Tags: frequency of each tag across all migrations
tag_counter = Counter()
if "tags" in df.columns:
    for tags_list in df["tags"].dropna():
        if isinstance(tags_list, list):
            tag_counter.update(tags_list)
tags_data = [{"tag": t, "count": c} for t, c in tag_counter.most_common()]
